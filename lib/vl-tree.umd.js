...
const set_hasOwnProperty = function (obj, key) {
...
class D_Set {
  ...
  add() {
    ...
    for (var i = 0; i < arguments.length; i++) {
      ...
      if (!set_hasOwnProperty(this.value, str)) {
        this.value[str] = val;
        this.n++; // 集合中值的计数加一
  ...
  has(value) {
    return set_hasOwnProperty(this.value, D_Set._v2s(value));
  } // 返回集合的大小
  ...
  forEach(f, context) {
    for (var s in this.value) {
      if (set_hasOwnProperty(this.value, s)) {
